const reducer1 = (state = [], action) =>  {
    ...
    const fetchResidents = async () => {
        ...
        data15.forEach((e) => {state.push(e.name)})
        data16.forEach((e) => {state.push(e.name)})
        data17.forEach((e) => {state.push(e.name)})
        data18.forEach((e) => {state.push(e.name)})
        data19.forEach((e) => {state.push(e.name)})        
    };
